...
public final class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public long addAlarm() {
        return addAlarm(new Reminder());
    }

    long addAlarm(Reminder reminder) {
        return getWritableDatabase().insert(TABLE_NAME, null, ReminderUtils.toContentValues(reminder));
    }

    public int updateAlarm(Reminder reminder) {
        final String where = _ID + "=?";
        final String[] whereArgs = new String[] { Long.toString(reminder.getId()) };
        return getWritableDatabase()
                .update(TABLE_NAME, ReminderUtils.toContentValues(reminder), where, whereArgs);
    }

    public int deleteAlarm(Reminder reminder) {
        return deleteAlarm(reminder.getId());
    }
